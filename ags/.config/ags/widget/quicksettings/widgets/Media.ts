...
function filterDuplicates(media_array: MprisPlayer[]) {
  const filtered: MprisPlayer[] = []
  for (const media of media_array) {
    print(media.track_title)
    if (filtered.find((m) => m.track_title === media.track_title)) continue
    filtered.push(media)
  }
  return filtered
}
...
export const Media = () =>
  Widget.Box({
    ...
    children: players.as((p) => filterDuplicates(p).map(Player)),
  })
