...
tmux send-keys -t nipro:link.1 "docker-compose run --rm --service-ports --name channel_host channel_host sh" 'Enter'
sleep 5
...
tmux send-keys -t nipro:link.1 "bin/hermes --host channel --host-config config/channels.yml --cloud-config config/cloud.yml" 'Enter'
sleep 5

echo "==> STARTING PROCESSORHOST THROUGH TMUX"
tmux send-keys -t nipro:link.2 "docker-compose run --rm --service-ports --name processor_host processor_host" 'Enter'
sleep 10

echo "==> STARTING SIMULATOR THROUGH TMUX"
tmux send-keys -t nipro:link.3 "docker-compose run --rm --service-ports --name sim dialysis_simulator" 'Enter'
